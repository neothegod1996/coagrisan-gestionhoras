...
import { Search, Plus, Edit, Trash2, MapPin, ArrowLeft } from "lucide-react";
...
export default function LocationsList({}: Props) {
  ...
  const handleGetLocations = async (filters: LocationFilters) => {
    setLocations({ ...locations, loading: true });
    getLocations(filters).then((response) => {
  ...
  const handleFilterChange = (key: keyof LocationFilters, value: string | boolean | undefined) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-brand rounded-md p-8 text-white">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-semibold">Gestión de Ubicaciones</h1>
              <p className="text-slate-200 mt-1">Administra las ubicaciones de tus centros</p>
            </div>
          </div>
          <Button
            onClick={() => setShowForm({ isOpen: true, type: "create", location_id: null })}
      ...
      <Card className="border-0 shadow-lg">
        <CardHeader className="pb-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold text-slate-800">Lista de Ubicaciones</h2>
          </div>
        </CardHeader>
                ...
                {locations.loading ? (
                  ...
                  locations.data.map((location) => (
                    <TableRow key={location.id} className="hover:bg-gray-50">
                      <TableCell className="font-medium">{location.id.slice(-10)}</TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <Badge 
                          className={`${location.status === "active" ? "bg-brand-primary-100 text-brand-primary" : "bg-gray-100 text-gray-800"}`}
                        >
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => setShowForm({ isOpen: true, type: "edit", location_id: location.id })}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <Pagination>
                <PaginationContent>
                  ...
                  {Array.from({ length: locations.total_pages }, (_, i) => i + 1).map((page) => (
                  ...
                  <PaginationItem>
                    <PaginationNext
                      onClick={() => handlePageChange(Math.min(locations.total_pages, filters.page + 1))}
                      className={filters.page === locations.total_pages ? "pointer-events-none opacity-50" : "cursor-pointer"}
                    />
                  </PaginationItem>
                </PaginationContent>
              </Pagination>
      ...
      <LocationForm
        ...
        onClose={() => setShowForm({ isOpen: false, type: "create", location_id: null })}
        location_id={showForm.location_id}
        refetch={() => handleGetLocations(filters)}
      />
    </div>
  );
}
